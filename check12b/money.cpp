...
{
    Money newMoney(this->getDollars() + rhs.getDollars(), this->getCents() + rhs.getCents());
    newMoney.handleOverflow();
    return newMoney;
}

Money& Money::operator+= (const Money &rhs)
{
    setDollars(this->getDollars() + rhs.getDollars());
    setCents(this->getCents() + rhs.getCents());
    handleOverflow();
    return *this ;
}
