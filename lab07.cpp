...
void partOne(int byValueParameter)
{
   ...
   pStack = &pStack; //address of local variable
   //malloc assigns new memory of size 0 and returns location
   pHeap = malloc(0); 
   pCode = (void*) &partOne; //address of function
   // ... and here
   ////////////////////////////////////////////////
...
void readStack()
{
   ...
   // using offset to set number to user number from partThree
   number = (long) * ( & (number) + 13); //18
 //using offset to get user text from partThree
   char* test = (char*)( & (number) + 9); 
  //*text = (char)*( & (number) + 14); //14
  strncpy(text,test,sizeof(text));
   
   //shared by bro wilson
   //find the function pointer referring to fail() on the stack
   long *p;
   for (p = (long *) &p; *p != (long)fail; p++)
   {
   }
   //change the function pointer to refer to pass() instead
   *p = (long)pass;

   //create a function pointer
   void (*pFun)(char);
   //now initialize it to the location of pass
   pFun = &pass;
   
//hack the end of the program by outputting what we want and quitting
 cout << "\tnumber: "
        << number
        << endl;
   cout << "\ttext:   "
