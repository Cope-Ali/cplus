...
void solveMaze()
{
   ...
   Graph graph = readMaze(fileName.c_str());
   vector <Vertex> path;
   drawMaze(graph, path);
   ...
   char temp;
   cin >> temp;
   //not sure where to get our starting and ending points?

   CVertex vFrom = graph.getStart();
   CVertex vTo = graph.getEnd();


  // vFrom.set(0, 0);
 //  vTo.set(4,4);
   
   //set <Vertex> vSet = graph.findPath(vFrom, vTo);
path = graph.findPath(vFrom, vTo);
