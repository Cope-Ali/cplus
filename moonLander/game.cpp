 ...
 *
 * ADDITIONAL FEATURES - Low fuel warning, out of fuel warning
...
void Game :: draw(const Interface & ui)
{
   ...
   if (lander.isLanded())
   {
      ...
      int score = 500 + lander.getFuel();
      Point scoreText;
      scoreText.setX(topLeft.getX() + 150);
      scoreText.setY(topLeft.getY() - 100);
      drawText(scoreText, "SCORE:");
      Point scoreNumber;
      scoreNumber.setX(scoreText.getX());
      scoreNumber.setY(scoreText.getY() -25 );
      drawNumber(scoreNumber, score);
   ...
   Point fuelWarning;
   fuelWarning.setX(topLeft.getX() + 100);
   fuelWarning.setY(topLeft.getY() - 25);
if (lander.getFuel()==0)
    drawText(fuelWarning, "You are out of fuel!");
if (lander.getFuel() <= 50 && lander.getFuel() > 0)
    drawText(fuelWarning, "Your fuel is low!");   
