 ...
 ***********************************************/
      ...
      void BST <T> :: insert(const T & t) throw (const char *)
   {
      ...
      else
      {
         ...
         // check and see if we stay red

        // already red, so Case 2 satisfied if nothing changes

         //these variables are for convenience to manipulate the aunt, parent, etc.
         BNode * pPar = pAdd->pParent;
         BNode * pGrand = NULL;
         BNode * pAunt = NULL;

         //here we set variables to correct positions
            ...
            //case 4
               if(pGrand != NULL)
               {
                  ...
                  if(pGrand->isRed == false)
                  {
                     ...
                     {
                        //aunt is black
                        if(pAunt->isRed == false)
                        {
                           if(pGrand->pLeft == pPar && pPar->pLeft == pAdd)
                           {
                              //if parent has right node, move to grandparent
                              if(pPar->pRight != NULL)
                              {
                                 pPar->pRight->pParent = pGrand;
                                 pGrand->pLeft = pPar->pRight;
                                 
                              }

                              //Move Parent into grand parent and make grand parent
                              //aunt
                              // pPar->pRight = pGrand;
                              ...
                              //Aunt is already right of grandparent 
