...
*    Estimated:  0.0 hrs   
*    Actual:     0.5 hrs
*      The most difficult part was figuring out exactly what it was I was
*    supposed to do. It was a little diffiult to step into someone elses
*    code and do the next part. This was good practice for real life!
************************************************************************/
...
float computeAverageLinear(int list[], int num)
{
   ...
   {
      search = list[i];
      count = linear(list,num,search);
      compares += count;
   }
   averageLinear = compares / (float)num;     
...
float computeAverageBinary(int list[], int num)
{
   ...
   {
      search = list[i];
      count = binary(list,num,search);
      compares += count;
   }
   averageBinary = compares / (float)num;     
